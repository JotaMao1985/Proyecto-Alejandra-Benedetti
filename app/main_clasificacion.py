...
def classify_document_hybrid(image: Image.Image, text_context: str) -> dict:
    base64_image = encode_image_to_base64(image)
    text_snippet = text_context[:1500].replace("\n", " ")

    # Qwen prefiere que le definas claramente la tarea visual
    prompt_sistema = f"""
    Tarea: Clasificación de Documentos Legales.
    
    Analiza la imagen proporcionada y el siguiente fragmento de texto extraído (OCR):
    "{text_snippet}"
    ...
    Instrucciones:
    1. Identifica el tipo de documento basándote en el encabezado visual y las palabras clave del texto.
    2. Responde ÚNICAMENTE con un objeto JSON. No expliques nada antes ni después.
    
    Formato JSON:
    {{
        "categoria": "CATEGORIA_ELEGIDA",
        "confianza": "ALTA/MEDIA/BAJA",
        "evidencia": "Texto breve encontrado que justifica la elección"
    }}
    """
...
def process_pdf(input_path: str) -> dict:
    """
    Orquesta el procesamiento de un archivo PDF individual.
    1. Abre el PDF.
    2. Extrae texto e imagen de la primera página.
    3. Invoca la clasificación híbrida.

    Args:
        input_path (str): Ruta al archivo PDF.
    ...
    try:
        ...
        if len(doc) == 0: 
            return {"categoria": "VACIO", "score": 0.0, "evidencia": "El archivo PDF no tiene páginas."}

        # Analizamos solo la primera página por eficiencia
        page = doc[0]
        
        # 1. Extracción de Texto
        text_content = page.get_text("text") or ""
        
        # 2. Renderizado de Imagen
        pix = page.get_pixmap(dpi=DPI)
        img = Image.open(io.BytesIO(pix.tobytes()))
        
        # 3. Pre-procesamiento visual
        processed_img = preprocess_image(img)
        
        # 4. Clasificación
        result = classify_document_hybrid(processed_img, text_content)
